...
open class UserManagerBaseTests: XCTestCase {
    open func userManager() -> SBUserManager? { ASUserManager() }
    
    // given information
    public let applicationId = "9ED4074F-D813-480F-A6DF-3D3E1FA1183D"   // Note: add an application ID
    public let apiToken = "c757f7e1f9d48cac34f39667ee6b9b44f803fc39"    // Note: add an API Token
    
    // profileImageURL is required as a parameter for creating users.
    private let profileImageURL = "https://grepp-programmers.s3.amazonaws.com/production/company/logo/3417/Sendbird_Symbol_RGB.png"
    ...
    public func testInitApplicationWithDifferentAppIdClearsData() throws {
        ...
        let initialUser = UserCreationParams(userId: userId, nickname: "hello", profileURL: profileImageURL)
    ...
    public func testCreateUser() throws {
        ...
        let params = UserCreationParams(userId: userId, nickname: userNickname, profileURL: profileImageURL)
        let expectation = self.expectation(description: "Wait for user creation")
    ...
    public func testCreateUsers() throws {
        ...
        let params1 = UserCreationParams(userId: userId1, nickname: userNickname1, profileURL: profileImageURL)
        let params2 = UserCreationParams(userId: userId2, nickname: userNickname2, profileURL: profileImageURL)
    ...
    public func testUpdateUser() throws {
        ...
        let initialParams = UserCreationParams(userId: userId, nickname: initialUserNickname, profileURL: profileImageURL)
        let updatedParams = UserUpdateParams(userId: userId, nickname: updatedUserNickname, profileURL: profileImageURL)
    ...
    public func testCreateUsersLimit() throws {
        ...
        let users = (0..<11).map { UserCreationParams(userId: "user_id_\(UUID().uuidString)\($0)",
                                                      nickname: "nickname_\(UUID().uuidString)\($0)",
        ...
        // 10.0 seconds could be short for testing in a simulater.
        // wait(for: [expectation], timeout: 10.0)
    ...
    public func testRateLimitCreateUser() throws {
        ...
        for _ in 0..<11 {
            dispatchGroup.enter()
            let params = UserCreationParams(userId: UUID().uuidString, nickname: UUID().uuidString, profileURL: profileImageURL)
            userManager.createUser(params: params) { result in
